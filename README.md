...
```swift
dependencies: [
    .package(url: "https://github.com/ninestar9552/ChaNetworkingSDK.git", from: "3.0.0")
]
```

`from: "3.0.0"`은 SwiftPM의 semantic versioning 규칙에 따라 최신 3.x 호환 버전을 사용합니다. 최신 릴리즈 번호는 Git tag를 기준으로 확인하세요.
...
// 3. 초기 로그인 후 토큰 쌍 저장
try client.tokenStorage.saveTokenPair(
    TokenPair(
...
asyncTokenRefresher: { currentRefreshToken in
    let response: AccessTokenRefreshResponse = try await authClient.post(
        "/auth/refresh",
        body: RefreshTokenRequest(refreshToken: currentRefreshToken)
    )
...
// 토큰 쌍 조회
if let storedTokens = storage.getTokenPair() {
...
final class CustomTokenStorage: TokenStorage {
    func saveTokenPair(_ tokenPair: TokenPair) throws { /* 구현 */ }
    func getTokenPair() -> TokenPair? { /* 구현 */ }
    func clearTokens() throws { /* 구현 */ }
}
```

3.0부터 `TokenStorage`는 Access Token과 Refresh Token을 항상 하나의 `TokenPair`로 저장·조회합니다. SDK의 `KeychainTokenStorage`는 토큰 쌍을 하나의 Keychain 항목으로 저장합니다.

`TokenStorage`는 credential 영속화만 담당하며 현재 로그인 상태를 판단하지 않습니다. 앱의 인증 세션 계층에서 다음 상태를 구분해야 합니다.

- 현재 로그인 여부: 로그인 또는 refresh 성공 후 확정한 세션 상태
- 자동 로그인 시도 가능 여부: 저장된 `TokenPair`가 있고 Refresh Token이 비어 있지 않은지 여부

저장된 토큰이 있다는 사실만으로 로그인 상태를 확정해서는 안 됩니다.
